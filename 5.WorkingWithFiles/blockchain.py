...
def load_data():
    with open('blockchain.txt', mode='rb') as f:  # read binary file
...
def save_data():
    with open('blockchain.p', mode='wb') as f:  # wb perché vogliamo salvare binary data
...
def valid_proof(transactions, last_hash, proof):
    ...
    print(guess_hash)
    # nel nostro caso abbiamo deciso che un valid hash deve avere 00 all'inizio (più zeri richiediamo più il PoW è difficile)
...
def get_balance(participant):
    ...
    # Ternary expression: tx_amt[0] if len(tx_amt) > 0 else 0
...
def mine_block():
    ...
    copied_transactions = open_transactions[:]  #  deep copy
...
while waiting_for_input:
    ...
    if not verify_chain():
        ...
        break
    print('Balance of {}: {:6.2f}'.format('Marco', get_balance('Marco')))
